...
NUM_SAMPLES = 2  # To make Tracy / CI faster
...
class MockDataLoader:
    def __init__(self, cfg, num_samples=2, seed=0):
        ...
        self.seed = seed
        self.dataset = [make_inputs(cfg, seed + i) for i in range(num_samples)]
    ...
    def __getitem__(self, idx):
        return {
            "img": [DataContainer(self.dataset[idx][0])],
            "img_metas": [DataContainer(self.dataset[idx][1])],
            "ego_fut_cmd": [DataContainer(self.dataset[idx][2])],
        }
...
def device():
    ...
    device_dict = {"device_id": 0, "l1_small_size": 29 * 1024}
    if DOUBLE_CQ:
...
def test_ssr_pcc(device):
    ...
    data_loader = MockDataLoader(cfg, num_samples=NUM_SAMPLES)
    ...
    logger.info("Run reference SSR-Net forward pass -skip")
    data = data_loader[-1]
    ...
    for i, data in enumerate(data_loader):
        logger.info(f"Running sample {i + 1}/{len(data_loader)}")
        ...
        # Clean up intermediate TT tensors after use
        if i != NUM_SAMPLES - 1:
            runner.dealloc_output()
    ttnn.synchronize_device(device)
