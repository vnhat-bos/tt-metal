...
"""
PyTest for the TT-NN ResNet-50 backbone.
...
def test_resnet_backbone_pcc(device):
    ...
    cfg.ttnn_model.img_backbone.debug = True
    tt_model = build_backbone(cfg.ttnn_model.img_backbone)
    tt_model.load_config_dict(module_config)
    tt_model = TTResNet50(tt_model)
    ...
    load_checkpoint(tt_model, TT_CHECKPOINT_PATH, map_location="cpu")

    # ----------------------- Compile ------------------------ #
    first_batch = next(iter(dataloader))

    # Torch (reference)
    logger.info("Run Resnet50 forward pass (Torch ref)")
    with torch.no_grad():
        data_pt = extract_data_from_container(deepcopy(first_batch), tensor="pt")
        ref_out = ref_model(
            data_pt["img"][0][0],
        )[-1]

    # TTNN
    logger.info("Run Resnet50 forward pass (TTNN)")
    data_tt = extract_data_from_container(
        deepcopy(first_batch), tensor="tt", device=device, input_config=cfg.input_config
    )
