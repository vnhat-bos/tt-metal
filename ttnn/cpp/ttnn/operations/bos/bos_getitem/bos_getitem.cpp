...
#include "bos_getitem.hpp"

namespace ttnn::operations::bos::bos_getitem {
Tensor BosGetItem::invoke(
    ...
    if (!input.has_value()) {
        ...
        TT_THROW("Input tensor is required for bos_getitem operation.");
    }
    return ttnn::prim::bos_getitem(input.value(), index_tensors, index_dims, output, memory_config);
}
}  // namespace ttnn::operations::bos::bos_getitem
