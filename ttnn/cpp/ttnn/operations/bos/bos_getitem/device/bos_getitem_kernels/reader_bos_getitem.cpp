...
void kernel_main() {
    ...
    uint64_t index_noc_base_addrs[5] = {
        get_noc_addr(0, index0),
        get_noc_addr(0, index1),
        get_noc_addr(0, index2),
        get_noc_addr(0, index3),
        get_noc_addr(0, index4),
    };

    volatile tt_l1_ptr int32_t* index_l1_ptrs[5] = {
        nullptr,
        nullptr,
        nullptr,
        nullptr,
    ...
    for (int32_t dim=3; dim >= 0; dim--) {
        if (index_is_defined[dim]) {
            ...
            uint64_t index_noc_addr = index_noc_base_addrs[dim];
    ...
    for (uint32_t i = start_id; i < end_id; ++i) {
        ...
        for (int32_t dim = 3; dim >= 0; dim--) {
            ...
            if (index_is_defined[dim]) {
                ...
                volatile tt_l1_ptr int32_t* index_l1_ptr = index_l1_ptrs[dim];
