...
void MAIN {
    ...
    uint32_t ntiles_per_core = get_arg_val<uint32_t>(1);
    ...
    constexpr uint32_t step_x = get_compile_time_arg_val(15);
    constexpr uint32_t step_y = get_compile_time_arg_val(16);
    constexpr uint32_t num_accum_points = get_compile_time_arg_val(17);
    constexpr uint32_t num_total_sticks = get_compile_time_arg_val(18);

    const float step_x_fp32 = (float)step_x;
    const float scaling_mean = 1.0f/batch_size;

    constexpr uint32_t MAX_TILES_PER_REDUCTION = 8;
    constexpr uint32_t MAX_POINTS_PER_REDUCTION = 4;
    ...
    uint32_t n_bilinear_steps = (num_accum_points + MAX_POINTS_PER_REDUCTION - 1)/MAX_POINTS_PER_REDUCTION;
    uint32_t stick_id = start_tile_id*32;
    uint32_t n_rows_per_face = n_bilinear_steps > 1 ? 16 : 4*num_accum_points;
    for (uint32_t i = 0; i < ntiles_per_core; i++) {
        ...
        fill_tile(1, step_x_fp32);     //dst[1] = step_x
        ...
        for (uint32_t row = 0; row < 32 && stick_id < num_total_sticks; row++, stick_id++){
            ...
            for (uint32_t bilinear_step_idx = 0; bilinear_step_idx < n_bilinear_steps; bilinear_step_idx++){
                DeviceZoneScopedN("fuse");
                cb_reserve_back(scaler_cbi, 1);
                cb_wait_front(bilinear_weight_cbi, 1);
                cb_wait_front(weight_cbi, 1);

                tile_regs_acquire();
                mul_tiles_init(bilinear_weight_cbi, weight_cbi);
                mul_tiles(bilinear_weight_cbi, weight_cbi, 0, 0, 0);
#ifdef is_QHB
                // temporal attention => we need to fuse mean into this
                fill_tile(1, scaling_mean);     //dst[1] = scaling_mean
                mul_binary_tile_init();
                mul_binary_tile(0, 1, 0);             //dst[0] *= scaling_mean
#endif
            ...
            tilizeA_B_reduce_init<false, true>(value_cbi, scaler_cbi, in_ntiles_c, out_cbi, 2, n_rows_per_face);
            pack_untilize_dest_init<in_ntiles_c>(out_cbi, 1, 2);
            cb_reserve_back(out_cbi, 1);
            tile_regs_acquire();
            for (uint32_t bilinear_step_idx = 0; bilinear_step_idx < n_bilinear_steps; bilinear_step_idx++){
                DeviceZoneScopedN("Bilinear and accumulate");
                cb_wait_front(value_cbi, n_rows_per_face);
                cb_wait_front(scaler_cbi, 1);

                // Value * scalar
                unpack_tilizeA_B_block<false, true, false, true>(value_cbi, scaler_cbi, in_ntiles_c, 0, 2, n_rows_per_face);

                for (uint32_t c_i = 0; c_i < in_ntiles_c; ++c_i)
                    reduce_tile_math(c_i, 2);

                cb_pop_front(value_cbi, n_rows_per_face);
