...
namespace ttnn::operations::bos::bos_deformable_attention {
    ...
    operation::ProgramWithCallbacks bos_deformable_attention_multi_core_interleaved(
        const Tensor& value,
        ...
        // B, num_keys, num_heads, embed_dims
        const auto& value_shape = value.logical_shape();
        // (B, num_queries, num_heads, embed_dims) or (B, num_queries, num_heads*embed_dims)
        ...
        uint32_t bilinear_weight_hash_unit_size = 0;
        ...
        if (use_bilinear_weight_hash) {
            ...
            bilinear_weight_hash_shape = bilinear_weight_hash_.logical_shape();  // steps_x, steps_y, 4
            bilinear_steps_x = bilinear_weight_hash_shape[0];
            bilinear_steps_y = bilinear_weight_hash_shape[1];
            ...
            bilinear_weight_hash_unit_size = bilinear_weight_hash_shape[-1] * bilinear_weight_hash_esize;   // bfloat16
            bilinear_weight_hash_addr = bilinear_weight_hash_buffer->address();
        }
        uint32_t bilinear_hash_size = bilinear_steps_x * bilinear_steps_y;

        //** 3. Split work to cores */
        auto compute_with_storage_grid_size = device->compute_with_storage_grid_size();
        uint32_t num_cores_x = compute_with_storage_grid_size.x;
        uint32_t num_cores_y = compute_with_storage_grid_size.y;
        uint32_t num_units = batch_size*num_queries*num_heads;
        auto [
            num_cores, all_cores,
        ...
        // Constants
        const uint32_t buffering_factor = num_levels*num_points;
        ...
        auto [value_cbi, value_cb] = CreateCB(value_cb_pagesize, 4*buffering_factor, value_cb_data_format, nullptr);
        auto [value_zero_cbi, value_zero_cb] = CreateCB(value_cb_pagesize, 4*buffering_factor, value_cb_data_format, nullptr);
        ...
        auto [weight_cbi, weight_cb] = CreateCB(weight_unit_size, 1, weight_cb_data_format, nullptr);
        ...
        auto [scaler_cbi, scaler_cb] = CreateCB(tile_shape*value_esize, 1*buffering_factor, value_cb_data_format, nullptr);
        auto [bilinear_weight_cbi, bilinear_weight_cb] = CreateCB(bilinear_weight_unit_size, 1, weight_cb_data_format, nullptr);

        // Output
        uint32_t num_units_out = value_shape[-1];
        uint32_t out_unit_size = num_units_out * output_esize;
        auto [out_cbi, out_cb] = CreateCB(out_unit_size, 1, output_cb_data_format, nullptr);
        ...
        std::string reader_kernel_path = use_bilinear_weight_hash
                                             ? "ttnn/cpp/ttnn/operations/bos/bos_deformable_attention/device/kernels/"
                                               "dataflow/bos_reader_deformable_attention_hash_interleaved.cpp"
                                             : "ttnn/cpp/ttnn/operations/bos/bos_deformable_attention/device/kernels/"
                                               "dataflow/bos_reader_deformable_attention_interleaved.cpp";
        ...
        std::string writer_kernel_path = use_bilinear_weight_hash
                                             ? "ttnn/cpp/ttnn/operations/bos/bos_deformable_attention/device/kernels/"
                                               "dataflow/bos_writer_deformable_attention_hash_interleaved.cpp"
                                             : "ttnn/cpp/ttnn/operations/bos/bos_deformable_attention/device/kernels/"
                                               "dataflow/bos_writer_deformable_attention_interleaved.cpp";
        ...
        std::string compute_kernel_path = "ttnn/cpp/ttnn/operations/bos/bos_deformable_attention/device/kernels/compute/bos_compute_deformable_attention_tile_interleaved.cpp";
        ...
        auto override_runtime_args_callback = [reader_kernel_id, compute_kernel_id, writer_kernel_id, cores, core_group_1, num_units_per_core_group_1, num_units_per_core_group_2, use_bilinear_weight_hash] (
                ...
                for (uint32_t i = 0; i < cores.size(); i++){
                    ...
                    compute_runtime_args[0] = n_units_per_core;
