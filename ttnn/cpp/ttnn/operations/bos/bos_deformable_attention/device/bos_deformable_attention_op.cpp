...
#include "ttnn/operations/bos/bos_deformable_attention/device/bos_deformable_attention_op.hpp"
#include "bos_deformable_attention_program_factory.hpp"
...
namespace ttnn::operations::bos::bos_deformable_attention{
    void BosDeformableAttention::validate_with_output_tensors(
    ...
    std::vector<TensorSpec> BosDeformableAttention::compute_output_specs(
        const std::vector<Tensor>& input_tensors,
        const std::vector<std::optional<Tensor>>& output_tensors
    ) const {
    ...
    std::vector<Tensor> BosDeformableAttention::create_output_tensors(
        const std::vector<Tensor>& input_tensors,
        const std::vector<std::optional<Tensor>>& output_tensors
    ) const {
        // already preallocate
    ...
    operation::ProgramWithCallbacks BosDeformableAttention::create_program(
        const std::vector<Tensor>& input_tensors,
        const std::vector<std::optional<const Tensor>>& optional_input_tensors,
        ...
        return bos_deformable_attention_multi_core_interleaved(
            value,
