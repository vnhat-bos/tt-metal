...
#include "bos_nd_reshard_program_factory.hpp"
...
operation::ProgramWithCallbacks bos_nd_reshard_multicore_copy_pages(const Tensor& input, Tensor& output) {
    ...
    KernelHandle reader_kernel_id = CreateKernel(
        program,
        "ttnn/cpp/ttnn/operations/bos/bos_sharded/bos_reshard/device/kernels/bos_nd_reshard_copy_pages_reader.cpp",
        grid,
        DataMovementConfig{
    ...
    KernelHandle writer_kernel_id = CreateKernel(
        program,
        "ttnn/cpp/ttnn/operations/bos/bos_sharded/bos_reshard/device/kernels/bos_nd_reshard_copy_pages_writer.cpp",
        grid,
        DataMovementConfig{
...
operation::ProgramWithCallbacks nd_reshard_multicore_copy_local_shard(
    ...
    KernelHandle brisc_kernel_id = CreateKernel(
        program,
        "ttnn/cpp/ttnn/operations/bos/bos_sharded/bos_reshard/device/kernels/bos_nd_reshard_copy_local_shards.cpp",
        grid,
        DataMovementConfig{
            .processor = DataMovementProcessor::RISCV_0,
            .noc = NOC::RISCV_0_default,
...
*/
operation::ProgramWithCallbacks bos_nd_reshard_multi_core(const Tensor& input, Tensor& output) {
    ...
    if (input_buffer_type == BufferType::DRAM && output_buffer_type == BufferType::DRAM) {
        return bos_nd_reshard_multicore_copy_pages(input, output);
    }
