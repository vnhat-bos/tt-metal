...
void BosReshardDeviceOperation::validate_with_output_tensors(
...
std::vector<ttnn::TensorSpec> BosReshardDeviceOperation::compute_output_specs(
    const std::vector<Tensor>& input_tensors, const std::vector<std::optional<Tensor>>& output_tensors) const {
...
BosReshardDeviceOperation::create_op_performance_model(
...
operation::ProgramWithCallbacks BosReshardDeviceOperation::create_program(
    const std::vector<Tensor>& input_tensors, std::vector<Tensor>& output_tensors) const {
    const auto& input_tensor = input_tensors.at(0);
    auto& output_tensor = output_tensors.at(0);
    if (CMAKE_UNIQUE_NAMESPACE::is_valid_for_legacy_reshard(input_tensor, output_tensor.memory_config())) {
        return bos_reshard_multi_core(input_tensor, output_tensor);
    } else {
        return bos_nd_reshard_multi_core(input_tensor, output_tensor);
    }
}

std::vector<Tensor> BosReshardDeviceOperation::create_output_tensors(
    const std::vector<Tensor>& input_tensors, const std::vector<std::optional<Tensor>>& output_tensors) const {
    if (output_tensors.size() == 1 && output_tensors[0].has_value()) {
