...
operation::ProgramWithCallbacks bos_non_zero_indices_single_core(
    ...
    // Number of indices to write/allocate in output derived from output tensor shape
    uint32_t output_elements = out_indices.padded_shape()[-1];
    ...
    [[maybe_unused]] bool src_is_dram = input.buffer()->buffer_type() == tt::tt_metal::BufferType::DRAM;
    [[maybe_unused]] bool out_is_dram_0 = out_num_indices.buffer()->buffer_type() == tt::tt_metal::BufferType::DRAM;
    [[maybe_unused]] bool out_is_dram_1 = out_indices.buffer()->buffer_type() == tt::tt_metal::BufferType::DRAM;
    ...
    uint32_t dst_page_size = output_elements * 4;
    ...
    auto kernel_id = tt::tt_metal::CreateKernel(
        program,
        "ttnn/cpp/ttnn/operations/bos/bos_non_zero_indices/device/kernels/dataflow/"
        "bos_non_zero_indices_sc_reader.cpp",
        core,
    ...
    auto override_runtime_args_callback = [kernel_id, core](
        ...
        uint32_t aligned_elements = tt::div_up(input.padded_shape()[-1], alignment_base) * alignment_base;
        uint32_t actual_elements = input.padded_shape()[-1];
        uint32_t output_elements = output_1.padded_shape()[-1];
        ...
        runtime_args[6] = output_elements;
    };
