...
void kernel_main() {
    ...
    uint32_t output_elements = get_arg_val<uint32_t>(6);
    ...
    uint32_t num_non_zero_indices = 0; // number written (capped by output_elements)
    uint32_t total_non_zero_found = 0; // total found in input
    uint32_t local_index = 0;
    for (uint32_t i = 0; i < actual_elements; i++) {
        if (input_addr_ptr[i] != 0) {
            if (num_non_zero_indices < output_elements) {
                indices_cb_id_index_addr_ptr[num_non_zero_indices] = i;
                num_non_zero_indices++;
            }
            total_non_zero_found++;
        }
        local_index++;
    }

    const uint32_t SENTINEL_VALUE = -1; // -1 unsigned
    for (uint32_t i = num_non_zero_indices; i < output_elements; i++) {
        indices_cb_id_index_addr_ptr[i] = SENTINEL_VALUE;
    }
    ...
    noc_async_write(indices_cb_id_index_addr, dst_noc_addr, output_elements * 4);
    noc_async_write_barrier();
}
